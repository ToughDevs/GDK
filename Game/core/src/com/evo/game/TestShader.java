...
public class TestShader implements Shader {
    ...
    public TestShader(boolean animal){
        super();
        this.animal = animal;
    }
    ...
    @Override
    public void init() {
        String vert = Gdx.files.internal("shaders/uvcolor.vertex.glsl")
                .readString();
        String frag = "";
        if(animal)
            frag += Gdx.files.internal("shaders/uvcolorAnimal.fragment.glsl")
                .readString();
        else
            frag += Gdx.files.internal("shaders/uvcolor.fragment.glsl")
                .readString();
        program = new ShaderProgram(vert, frag);
        if (!program.isCompiled())
            throw new GdxRuntimeException(program.getLog());
    ...
    public void render(Renderable renderable) {
        ...
        renderable.meshPart.mesh.render(program, renderable.meshPart.primitiveType,
                renderable.meshPart.offset, renderable.meshPart.size);
    }
